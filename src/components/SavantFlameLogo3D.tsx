...
/**
 * BLUE_FLAME_SHADER // SOVEREIGN_AETHER_V4
 */
...
const WarpTunnel = ({ active }: { active: boolean }) => {
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < count; i++) {
        const i3 = i * 3;
        positions[i3 + 2] += speeds[i] * 4;
        if (positions[i3 + 2] > 200) positions[i3 + 2] = -300;
    }
...
const AlienRing = ({ progress, visible }: { progress: number, visible: boolean }) => {
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < count; i++) {
        const i3 = i * 3;
        positions[i3 + 2] = Math.sin(t * 2 + i * 0.1) * 2 * (progress / 100);
    }
    pointsRef.current.geometry.attributes.position.needsUpdate = true;
...
const PreloaderScene = ({ phase, narrative, progress, initialized, booted, bloomBoost, isMobile }: { phase: string, narrative: string, progress: number, initialized: boolean, booted: boolean, bloomBoost: number, isMobile: boolean }) => {
  ...
  useFrame((state) => {
    ...
    if (initialized) {
        // HEADER_LOGU_MODE: Fly to top left corner
        const scale = 0.5; // Small standard logo size
        groupRef.current.scale.lerp(new THREE.Vector3(scale, scale, scale), 0.1);
        
        // Diagonal route to corner
    ...
    if (phase === 'loading' || phase === 'ready') {
       ...
       else if (narrative === '03_CHAMBER_DRIFT') targetPos.set(-20, 4, 30);
       else if (narrative === '04_CORE_SYNC') targetPos.set(Math.sin(t * 40) * 0.8, Math.cos(t * 40) * 0.8, 20);
       else if (narrative === '05_CALIBRATION') targetPos.set(0, 0, 50);
       else if (narrative === '06_STABILIZED') targetPos.set(0, 0, 12); // Slightly closer
       ...
       if (isStab) {
          // Final Atmospheric Float - Facing Camera
          groupRef.current.position.y = Math.sin(t * 1.5) * 0.6;
          groupRef.current.rotation.y = t * 2.5; // Faster rotation
...
export default function SavantFlameLogo3D() {
  ...
  const { setBooted, booted, initializing, setInitializing } = useStore();
  const tlRef = useRef<gsap.core.Timeline | null>(null);
  const quality = useAdaptiveQuality();

  useEffect(() => {
    if (booted || tlRef.current) return;
    ...
    const ctx = gsap.context(() => {
      ...
      tl.to({}, { duration: 1.0, onStart: () => setNarrative('00_INITIATION') });
      tl.to({}, { duration: 0.8, onStart: () => setNarrative('01_SINGULARITY') });
      tl.to({}, { duration: 2.5, onStart: () => setNarrative('02_PROPULSION') });
      tl.to({}, { duration: 1.8, onStart: () => setNarrative('03_CHAMBER_DRIFT') });
      tl.to({}, { duration: 1.5, onStart: () => setNarrative('04_CORE_SYNC') });
      tl.to({}, { duration: 0.4, onStart: () => setNarrative('05_CALIBRATION') });
      tl.to({}, { duration: 2.5, onStart: () => setNarrative('06_STABILIZED') });
    });
  ...
  return (
    ...
    >
      ...
      >
        <Suspense fallback={null}>
          <PreloaderScene phase={phase} narrative={narrative} progress={percent} initialized={isTransitioning} booted={booted} bloomBoost={quality.bloomIntensity} isMobile={quality.isMobile} />
        </Suspense>
      </Canvas>
