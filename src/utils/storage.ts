...
export const saveImagesToDB = async (key: string, images: string[]) => {
  try {
...
export const loadImagesFromDB = async (key: string): Promise<string[] | null> => {
  try {
    const db = await initDB();
    return new Promise((resolve, reject) => {
