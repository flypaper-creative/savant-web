...
class LogoSystem {
  constructor(scene) {
    ...
    this.tracerRig = new THREE.Group();
    this.group.add(this.tracerRig);
    this.tracerCurve = null;
    this.tracerLight = null;
    this.tracerHead = null;
    this.tracerCore = null;
    this.tracerShell = null;
    this.tracerTrail = [];
    this.tracerRig = new THREE.Group();
    this.group.add(this.tracerRig);
    this.tracerLight = null;
  ...
  update(progress, time) {
    ...
    if (p <= 0.60) {
      ...
      this.group.rotation.x += (lerp(-0.56, -0.05, t1) - this.group.rotation.x) * 0.16;
      this.group.rotation.y += (lerp(0.34, 0.02, t1) - this.group.rotation.y) * 0.16;
      ...
      this.root.scale.lerp(this.tmpScale, 0.18);
      this.root.rotation.y += 0.00002;
    } else {
      ...
      this.group.position.lerp(pos, 0.20);

      this.group.rotation.x += (lerp(-0.05, -0.015, t2) - this.group.rotation.x) * 0.18;
      this.group.rotation.y += (lerp(0.02, 0.00, t2) - this.group.rotation.y) * 0.18;
      this.group.rotation.z += (0.0 - this.group.rotation.z) * 0.18;

      const holdMul = lerp(1.26, 1.02, t2);
      this.tmpScale.setScalar(this.baseScale * holdMul);
      this.root.scale.lerp(this.tmpScale, 0.18);

      const settledRotationSpeed = t2 < 0.50 ? 0.00060 : lerp(0.01050, 0.02550, clamp((t2 - 0.50) / 0.50, 0, 1));
      this.root.rotation.y += settledRotationSpeed;
      this.root.rotation.x = Math.sin(time * 0.08) * 0.0008;
    }
  }
}
