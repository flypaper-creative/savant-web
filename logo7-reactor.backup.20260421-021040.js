...
const CFG = {
  ...
  ringRadius: 2.64,
  ringCenter: new THREE.Vector3(0, 0, -10.95),
  cameraZ: 7.15,
  logoTargetSize: 3.08,
  ...
  dpr: Math.min(window.innerWidth < 900 ? 1.2 : 1.5, window.devicePixelRatio || 1.4)
};
...
function blackGlassMat(color = 0x07080a) {
  return new THREE.MeshPhysicalMaterial({
    color,
    metalness: 0.56,
    roughness: 0.055,
...
const key = new THREE.DirectionalLight(0xfff5eb, 0.96);
key.position.set(4.4, 4.8, 3.6);
scene.add(key);

const fill = new THREE.DirectionalLight(0xe4eeff, 0.34);
fill.position.set(-4.0, 1.6, 2.4);
scene.add(fill);

const warmBack = new THREE.PointLight(0xffab43, 1.52, 24, 2);
warmBack.position.set(0, 0.06, CFG.ringCenter.z + 1.8);
scene.add(warmBack);

const coldSpec = new THREE.PointLight(0xf9fbff, 0.56, 16, 2);
coldSpec.position.set(0.2, 2.2, -7.2);
scene.add(coldSpec);

const ledSweepLight = new THREE.PointLight(0xffb65e, 0.0, 15, 2);
scene.add(ledSweepLight);

const topBladeLight = new THREE.PointLight(0xffd99d, 2.8, 20, 2);
topBladeLight.position.set(0, 2.78, -9.72);
scene.add(topBladeLight);

const logoAccentLight = new THREE.PointLight(0xfff6ea, 0.58, 12, 2);
logoAccentLight.position.set(0, 0.28, -8.4);
scene.add(logoAccentLight);

/* haze */
const chamberHaze = makeGlowSprite([
  [0.00, 'rgba(255,205,132,0.18)'],
  [0.10, 'rgba(255,138,34,0.12)'],
...
class DustField {
  constructor(scene) {
    ...
    const sizes = new Float32Array(CFG.dustCount);
    ...
    for (let i = 0; i < CFG.dustCount; i++) {
      ...
      positions[i * 3 + 1] = y;
      positions[i * 3 + 2] = CFG.ringCenter.z + (Math.random() - 0.5) * 3.4;
...
class RingShards {
  constructor(scene) {
    ...
    for (let i = 0; i < CFG.shardCount; i++) {
      ...
      mesh.userData = {
        radius: CFG.ringRadius + 0.45 + Math.random() * 1.0,
        speed: 0.18 + Math.random() * 0.4,
        phase: Math.random() * Math.PI * 2,
        yAmp: 0.18 + Math.random() * 0.4
      };
      this.group.add(mesh);
...
class RingSystem {
  constructor(scene) {
    ...
    this.outer = new THREE.Mesh(
      new THREE.TorusGeometry(CFG.ringRadius, 0.14, 28, 300),
      metalMat(0x040506, 0.032, 1.0, 1.0, 3.2)
    );
    this.rig.add(this.outer);

    this.midArmor = new THREE.Mesh(
      new THREE.TorusGeometry(CFG.ringRadius + 0.012, 0.065, 18, 260),
      metalMat(0x090b10, 0.048, 1.0, 1.0, 2.9)
    );
    this.rig.add(this.midArmor);

    this.innerRail = new THREE.Mesh(
      new THREE.TorusGeometry(CFG.ringRadius - 0.17, 0.028, 18, 260),
      metalMat(0x181a1f, 0.022, 1.0, 1.0, 3.0)
    );
    this.rig.add(this.innerRail);

    this.coreLine = new THREE.Mesh(
      new THREE.TorusGeometry(CFG.ringRadius - 0.22, 0.012, 14, 220),
      emissiveMat(0xffce7a, 0.06)
    );
    this.rig.add(this.coreLine);

    this.haloRing = new THREE.Mesh(
      new THREE.TorusGeometry(CFG.ringRadius - 0.22, 0.022, 14, 180),
  ...
  buildArmor() {
    const geo = new THREE.BoxGeometry(0.44, 0.18, 0.16);
    ...
    for (let i = 0; i < CFG.armorCount; i++) {
      ...
      s.set(1.0, i % 4 === 0 ? 1.8 : (i % 2 ? 1.18 : 1.42), 1.0);
      p.set(Math.cos(a) * (CFG.ringRadius + 0.02), Math.sin(a) * (CFG.ringRadius + 0.02), 0.11);
      q.setFromEuler(new THREE.Euler(0, 0, a));
      m.compose(p, q, s);
      this.armor.setMatrixAt(i, m);
      c.setHex(i % 2 ? 0x0f1116 : 0x06070a);
  ...
  buildLEDs() {
    ...
    const housingGeo = new THREE.BoxGeometry(0.082, 0.05, 0.05);
    ...
    const emitterGeo = new THREE.BoxGeometry(0.05, 0.02, 0.012);
    ...
    for (let i = 0; i < CFG.ledCount; i++) {
      ...
      const radius = CFG.ringRadius - 0.14;

      p.set(Math.cos(a) * radius, Math.sin(a) * radius, 0.15);
      q.setFromEuler(new THREE.Euler(0, 0, a));
      m.compose(p, q, s);
      this.ledHousing.setMatrixAt(i, m);
      this.ledHousing.setColorAt(i, darkHousing);

      p.set(Math.cos(a) * radius, Math.sin(a) * radius, 0.176);
      m.compose(p, q, s);
  ...
  buildEmitters() {
    ...
    const bodyGeo = new THREE.BoxGeometry(0.36, 0.07, 0.1);
    ...
    for (let i = 0; i < CFG.emitterCount; i++) {
      ...
      const a = (i / CFG.emitterCount) * Math.PI * 2 + (i % 2 ? 0.04 : -0.025);
      g.position.set(Math.cos(a) * (CFG.ringRadius + 0.04), Math.sin(a) * (CFG.ringRadius + 0.04), 0.12);
      g.rotation.z = a;

      const body = new THREE.Mesh(bodyGeo, bodyMat);
      body.scale.y = i % 3 === 0 ? 1.55 : (i % 2 ? 0.84 : 1.18);
      g.add(body);

      const slit = new THREE.Mesh(
        new THREE.BoxGeometry(0.26, 0.016, 0.014),
        new THREE.MeshBasicMaterial({
          color: 0xffcb75,
          transparent: true,
          opacity: 0,
  ...
  buildTopBlade() {
    ...
    const housing = new THREE.Mesh(
      new THREE.BoxGeometry(0.30, 0.52, 0.18),
      metalMat(0x06070a, 0.03, 1.0, 1.0, 3.2)
    );
    this.topBladeRig.add(housing);

    this.topBladeCore = new THREE.Mesh(
      new THREE.BoxGeometry(0.065, 0.72, 0.03),
      new THREE.MeshBasicMaterial({
        color: 0xffdc9b,
        transparent: true,
        opacity: 0.95,
  ...
  update(progress, time) {
    ...
    this.tmpGold.setRGB(2.65, 1.5, 0.46);
    ...
    for (let i = 0; i < CFG.ledCount; i++) {
      ...
      if (level > 0.001) {
        this.ledEmitter.setColorAt(i, this.tmpMix.clone().multiplyScalar(level * 4.9));
    ...
    ledSweepLight.position.z += 0.56;
    ledSweepLight.intensity = 1.35 + frac * 4.0;
    ledSweepLight.distance = 13.0 + frac * 5.6;

    const bladePulse = 0.86 + Math.sin(time * 5.1) * 0.12 + p * 0.2;
    this.topBladeCore.material.opacity = bladePulse;
    this.topBladeRig.rotation.z = Math.sin(time * 0.42) * 0.012;

    this.outer.rotation.z += 0.00055;
    this.midArmor.rotation.z -= 0.00032;
    this.innerRail.rotation.z += 0.0011;
    this.coreLine.rotation.z -= 0.0016;
    this.haloRing.rotation.z += 0.0008;
    ...
    this.rig.rotation.z = Math.sin(time * 0.16) * 0.011;
    this.rig.rotation.x = Math.sin(time * 0.14) * 0.017;
  }
}
...
class LogoSystem {
  constructor(scene) {
    ...
    this.start = new THREE.Vector3(0, 9.5, -1.15);
    this.cp1 = new THREE.Vector3(0, 6.7, -5.2);
    this.cp2 = new THREE.Vector3(0, 1.45, -9.8);
    this.mid = new THREE.Vector3(0, 0.09, -10.32);
  ...
  makeObsidianMaps() {
    const roughnessMap = makeCanvasTexture((g, w, h) => {
      ...
      for (let i = 0; i < img.data.length; i += 4) {
        const v = Math.floor(8 + Math.random() * 24);
        img.data[i] = v;
        img.data[i + 1] = v;
        img.data[i + 2] = v;
        img.data[i + 3] = 255;
    ...
    const bumpMap = makeCanvasTexture((g, w, h) => {
      ...
      for (let i = 0; i < 1800; i++) {
        ...
        const v = Math.floor(90 + Math.random() * 58);
  ...
  applyMaterial(root) {
    ...
    root.traverse((obj) => {
      ...
      obj.material = new THREE.MeshPhysicalMaterial({
        ...
        bumpScale: 0.012,
        specularIntensity: 1.0,
        specularColor: new THREE.Color(0xfdf4e7)
      });
    });
  }
  ...
  update(progress, time) {
    ...
    if (p <= 0.62) {
      ...
      const pos = cubicBezier3(
        this.mid,
        new THREE.Vector3(0, 0.08, -11.02),
...
function animate() {
  ...
  const cameraYaw = 5.7 * Math.PI / 180;
